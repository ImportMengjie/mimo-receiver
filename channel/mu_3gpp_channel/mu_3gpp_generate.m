...
sequenceLength = timeslots;                % Number of steps in a track to simulate for each drop

K = n_t;                              % Number of users
disp('32 users')
...
H_ = cell2mat(h');
clear h
H = zeros(numDrops*sequenceLength, numSubcarriers/subSampling, M, K, 2);
H_ = permute(H_, [1,5,4,3,2]);

for i=0:numDrops*sequenceLength-1
    H(i+1,:,:,:,1) = real(H_(floor(i/sequenceLength)+1,mod(i,sequenceLength)+1,:,:,:));
    H(i+1,:,:,:,2) = imag(H_(floor(i/sequenceLength)+1,mod(i,sequenceLength)+1,:,:,:));
end

file_path = sprintf('%s/../../data/3gpp_mu_%d_%d_%d_%d_%d.mat',fileparts(mfilename('fullpath')),n_r, n_t, n_sc, timeslots, numDrops);
